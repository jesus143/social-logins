 ...
 *
 * @since  	    Sunday, June 22, 2014, 06:14 PM GMT+1
 * @modified    $Date: 2014-06-24 10:19:35 +0200 (Tue, 24 Jun 2014) $ $Author: dev@cloneui.com $
 * @version     $Id: Tumblr.php 13 2014-06-24 08:19:35Z dev@cloneui.com $
 *
 * @category    External Authenticaton
 * @package     AuthAny
*/

require_once( dirname( AUTHANY_ROOT ).'/Tumblr.php' );
class AuthAny_Provider_Tumblr extends AuthAny
{
	private $_apiConfig;
	private $_envConfig;
	private $_provider;
	...
	public function handleAuth()
	{
		if( strlen( @$_COOKIE[$this->_provider.'_access_token'] ) AND strlen( @$_COOKIE[$this->_provider.'_access_secret'] ) ) {
			...
			if( empty( $_REQUEST ) ) {
				...
				$Tumblr = new Tumblr( $this->_envConfig->key, $this->_envConfig->secret );
				 
				// Ask Tumblr for a Request Token
				$request_token = $Tumblr->getRequestToken( AUTHANY_BASEURL.'/'.$this->_envConfig->redirectUri );
				...
				switch ( $Tumblr->http_code ) {
					case 200:
						...
						$url = $Tumblr->getAuthorizeURL( $request_token['oauth_token'] );
					...
					default:
						...
						trigger_error( 'Could not connect to Tumblr. Refresh the page or try again later. '.var_export( $request_token, true ), E_USER_ERROR );
				}
			} else {
				...
				// Create an instance of Tumblr_OAuth.
				// It'll need our Consumer Key and Secret as well as our Request Token and Secret
				$Tumblr = new Tumblr( $this->_envConfig->key, $this->_envConfig->secret, $_SESSION[$this->_provider]['request_token'], $_SESSION[$this->_provider]['request_token_secret'] );
	...
	*/	
	public function handleAccessToken( $data = array() ) 
	{
		if( isset( $data['access_token'] ) AND isset( $data['access_secret'] ) ) {
			...
			$user = $this->getUserDetails( $Tumblr );
	...
	*/
	 ...
	 * @return	array
	*/	
	public function getUserDetails( Tumblr $config )
	{				
		// get user info
		$userInfo = $config->get('user/info');
					
		if( !empty( $userInfo->response->user ) ) {			
			return $this->objectToArray( $userInfo->response->user );
		} else {
			return array();
		}
	}
