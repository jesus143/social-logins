 ...
 * @link		https://disqus.com/api/docs/permissions
 * @link		https://disqus.com/api/docs/requests
 * @link		https://disqus.com/api/docs/users/details
 * 
 * @license     Commercial
 * @since  	    Tuesday, June 24, 2014, 09:16 AM GMT+1
 * @modified    $Date: 2014-06-24 10:19:35 +0200 (Tue, 24 Jun 2014) $ $Author: dev@cloneui.com $
 * @version     $Id: Disqus.php 13 2014-06-24 08:19:35Z dev@cloneui.com $
 *
 * @category    External Authenticaton
 * @package     AuthAny
*/

class AuthAny_Provider_Disqus extends AuthAny
{
	private $_apiConfig;
	private $_envConfig;
	private $_provider;
	...
	*/	
	...
	public function getAccessToken( $code )
	{
		if( strlen( $code ) ) {
			$params = array(
				'client_id'		=> $this->_envConfig->apiKey,
				'client_secret'	=> $this->_envConfig->apiSecret,
				'code'			=> $code,
				'redirect_uri'	=> AUTHANY_BASEURL.'/'.$this->_envConfig->redirectUri,
	...
	*/	
	...
	{			
		$params = array(
			...
			'api_key'		=> $this->_envConfig->apiKey,
			'api_secret'	=> $this->_envConfig->apiSecret
		);			
					
		$apiEndpoint = $this->_apiConfig->apiEndpoint.'/users/details.json?'.http_build_query( $params );
