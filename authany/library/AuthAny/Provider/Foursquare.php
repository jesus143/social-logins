 ...
 * 
 * @link		https://developer.foursquare.com/overview/auth.html
 * @link		https://developer.foursquare.com/docs
 * 
 * @license     Commercial
 * @since  	    Tuesday, June 24, 2014, 09:01 AM GMT+1
 * @modified    $Date: 2014-06-24 10:19:35 +0200 (Tue, 24 Jun 2014) $ $Author: dev@cloneui.com $
 * @version     $Id: Foursquare.php 13 2014-06-24 08:19:35Z dev@cloneui.com $
 *
 * @category    External Authenticaton
 * @package     AuthAny
*/

class AuthAny_Provider_Foursquare extends AuthAny
{
	private $_apiConfig;
	private $_envConfig;
	private $_provider;
	...
	*/	
	...
	{
		$apiEndpoint = $this->_apiConfig->apiEndpoint.'/users/self?oauth_token='.$accessToken.'&v='.$this->_apiConfig->version;
		$apiResponse = json_decode( $this->curl_get_url( $apiEndpoint ) );
		$apiResponse = $this->objectToArray( $apiResponse );
		$apiResponse = $apiResponse['response']['user'];
		
		if( isset( $apiResponse['contact']['email'] ) ) {
			$apiResponse['email'] = $apiResponse['contact']['email']; 	
		}
